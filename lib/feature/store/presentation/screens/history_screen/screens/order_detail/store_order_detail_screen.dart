...
class StoreOrderDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) =>
          getIt<StoreOrderDetailBloc>()
            ..add(StoreOrderDetailStarted(orderId: orderId)),
...
class _StoreOrderDetailViewState extends State<_StoreOrderDetailView>
    with StoreOrderDetailScreenMixin<_StoreOrderDetailView> {
    ...
    return BlocBuilder<StoreOrderDetailBloc, StoreOrderDetailState>(
      builder: (context, state) {
        return Scaffold(
          ...
          body: SafeArea(
            top: state.status != StoreOrderDetailStatus.success,
            bottom: false,
            child: switch (state.status) {
              StoreOrderDetailStatus.loading ||
              StoreOrderDetailStatus.initial => const Center(
                child: Skeletonizer(child: _OrderDetailSkeleton()),
              ),
              StoreOrderDetailStatus.failure => TgsFailureContent(
                message: "Buyurtmani yuklashda xatolik",
                onRetry: () => context.read<StoreOrderDetailBloc>().add(
                  const StoreOrderDetailRetryRequested(),
                ),
              ),
              StoreOrderDetailStatus.success => _buildContent(
                context,
                state.order!,
              ),
            },
          ),
          bottomNavigationBar: state.status == StoreOrderDetailStatus.success
  ...
  Widget _buildContent(BuildContext context, StoreOrderModel order) {
    final media = order.productMedia
        .map((url) => StoreMediaModel(id: url, url: url))
    ...
    return CustomScrollView(
      slivers: [
        SliverAppBar(
          ...
          title: _CollapsingAppBarTitle(
            title: order.productTitle.isNotEmpty
                ? order.productTitle
                : 'Mahsulot',
          ),
          flexibleSpace: Stack(
                    ...
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Text(
                            CoinCompactFormat.short(order.unitPrice),
                    ...
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          const SizedBox(width: 4),
                          Text(
                            _resolveStatusLabel(order.status),
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: StoreDetailInfoCard(
                ...
                features: _extractFeatures(order.productDescription!),
              ),
            ),
          ),
        SliverToBoxAdapter(
    ...
    return StoreDetailPurchaseBar(
      ...
      onPressed: isPromo && hasPromoCode
          ? () => showAppBottomSheet<void>(
              context,
              child: StorePromoCodeSheet(
                promoCode: order.promoCode!,
                productTitle: order.productTitle.isEmpty
                    ? 'Promo kod'
                    : order.productTitle,
              ),
            )
  ...
  Future<void> _openPickupInYandexMaps(BuildContext context) async {
    final yandexAppUri = Uri.parse(
      'yandexmaps://maps.yandex.com/?pt=$_pickupLon,$_pickupLat&z=16',
    );
    final yandexWebUri = Uri.parse(
      'https://yandex.com/maps/?pt=$_pickupLon,$_pickupLat&z=16',
    );
