...
class StoreHistoryScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(create: (_) => getIt<StoreHistoryBloc>()..add(const StoreHistoryStarted()), child: const _StoreHistoryView());
  }
}
...
class _StoreHistoryViewState extends State<_StoreHistoryView> with StoreHistoryScreenMixin<_StoreHistoryView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        bottom: false,
        child: BlocConsumer<StoreHistoryBloc, StoreHistoryState>(
          listenWhen: (previous, current) => current.loadMoreFailed && !previous.loadMoreFailed,
          listener: storeHistoryBlocListener,
          builder: (context, state) {
            final isInitialLoading = (state.status == StoreHistoryStatus.loading || state.status == StoreHistoryStatus.initial) && state.items.isEmpty;
                    ...
                    children: [
                      Row(
                        children: [
                          ...
                          Text('Mahsulotlar tarixi', style: context.textTheme.bodyXLargeSemibold.copyWith(color: context.appColors.text)),
                        ],
                      ),
                    ],
                  ),
                  ...
                  child: switch (state.status) {
                    ...
                    StoreHistoryStatus.success when state.items.isEmpty => Center(
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            Icon(LucideIcons.shoppingCart, size: 48, color: context.appColors.grey),
                    ...
                    _ => NotificationListener<ScrollNotification>(
                      ...
                      child: AppStaggeredScrollLimiter(
                        key: ValueKey('store-history-grid-${state.pageNumber}-${state.items.length}'),
                        child: GridView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return AppStaggeredListItem(
                              position: index,
                              child: StoreOrderHistoryGridItem(order: order, onTap: () => onOrderTap(context, order.id)),
                            );
                          },
                        ),
                      ),
