...
double _pulseFromProgress(double value) {
...
Widget _slidingTabIndicator({required double x, required double width, required Color baseColor, required bool lightBar, required double indicatorBlurSigma}) {
  final bool useIndBlur = indicatorBlurSigma > 0;
  final Color softer = Color.lerp(baseColor, lightBar ? const Color.fromARGB(255, 0, 0, 0) : AppColors.white, 0.12)!;
  if (useIndBlur) {
        ...
        child: BackdropFilter(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: softer.withValues(alpha: lightBar ? 0.08 : 0.1),
              borderRadius: BorderRadius.circular(999),
            ),
          ),
        ),
...
/// Qoshimcha tugma: [LiquidBottomNav] trubasi bilan bir xil blur, stroke va soya (doira).
class _LiquidNavExtraIconButton extends StatelessWidget {
  const _LiquidNavExtraIconButton({
  ...
  Widget build(BuildContext context) {
    ...
    final Color fillColor = useBlur ? backgroundColor.withValues(alpha: lightBar ? 0.48 : 0.58) : backgroundColor;
    final Color barShadow = AppColors.shadow.withValues(alpha: lightBar ? 0.14 : 0.24);
...
/// [LiquidBottomNav] ranglari: `Color?` ishlatilganda yoki barcha `null` qoldirilganda
/// [liquidBottomNavThemePalette] yordamida olinadi.
class LiquidBottomNavThemePalette {
  const LiquidBottomNavThemePalette({required this.backgroundColor, required this.selectedColor, required this.unselectedColor, required this.indicatorColor});
...
/// Light: qora truba, oq surilmali pill, tanlangan (pill ustida) qora, tanlanmagan oq/och;
/// Dark: oq truba, to‘q surilmali pill, tanlangan oq, tanlanmagan qora/och.
LiquidBottomNavThemePalette liquidBottomNavThemePalette(BuildContext context) {
  if (Theme.of(context).brightness == Brightness.dark) {
    return LiquidBottomNavThemePalette(backgroundColor: context.appColors.onContainer, selectedColor: AppColors.black, unselectedColor: Color(0xB3FFFFFF), indicatorColor: AppColors.white);
  }
  return LiquidBottomNavThemePalette(
...
class LiquidBottomNavItem {
  const LiquidBottomNavItem({this.icon, this.activeIcon, this.iconBuilder, required this.label, this.tooltip})
    : assert(icon != null || iconBuilder != null, 'LiquidBottomNavItem requires icon or iconBuilder');
...
class LiquidBottomNav extends StatefulWidget {
  const LiquidBottomNav({
    ...
    /// Orqa kontentni shisha effekti: `0` — blur yo‘q (to‘liq [`backgroundColor`]).
...
class _LiquidBottomNavState extends State<LiquidBottomNav> {
  ...
  @override
  Widget build(BuildContext context) {
    final p = liquidBottomNavThemePalette(context);
    final Widget? resolvedExtra = widget.extraActionIcon != null
        ? _LiquidNavExtraIconButton(
    ...
    return SizedBox(
      height: widget.height + widget.margin.vertical + 24,
      child: Stack(
                    ...
                    child: Listener(
                      ...
                      child: SingleMotionBuilder(
                        ...
                        builder: (context, animatedTick, child) {
                          final double pulse = _pulseFromProgress(animatedTick);
...
class _PillWithTabs extends StatefulWidget {
  const _PillWithTabs({
  ...
  @override
  State<_PillWithTabs> createState() => _PillWithTabsState();
}

class _PillWithTabsState extends State<_PillWithTabs> {
                ...
                children: [
                  Positioned.fill(
                    child: Align(
                      ...
                      child: _slidingTabIndicator(x: x, width: indicatorWidth, baseColor: widget.indicatorColor, lightBar: widget.lightBar, indicatorBlurSigma: widget.indicatorBlurSigma),
                    ),
                  ),
                  Row(
  ...
  Widget build(BuildContext context) {
    return SingleMotionBuilder(
      ...
      builder: (context, animatedTick, child) {
        ...
        final bool useBlur = backgroundBlurSigma > 0;
        final double sigma = backgroundBlurSigma;
        final Color fillColor = useBlur ? dynamicBackground.withValues(alpha: lightBar ? 0.48 : 0.58) : dynamicBackground;
...
class _NavItem extends StatelessWidget {
  const _NavItem({required this.item, required this.isSelected, required this.onTap, required this.iconSize, required this.selectedColor, required this.unselectedColor});

  final LiquidBottomNavItem item;
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                softWrap: false,
                style: TextStyle(color: contentColor, fontSize: 10, fontWeight: isSelected ? FontWeight.w700 : FontWeight.w500, height: 1),
              ),
            ],
          ),
        ),
...
/// yoki barcha ranglarni o‘tmasdan default [liquidBottomNavThemePalette] ishlatiladi.
List<LiquidBottomNavItem> mainAppLiquidBottomNavItems(BuildContext context, {required bool isGuestMode}) {
  final l10n = context.l10n;
  final appColors = context.appColors;
  return [
    LiquidBottomNavItem(label: l10n.mainTabHome, iconBuilder: (_, color, size, selected) => MainBottomNavKitIcons.home(color, size, selected)),
    LiquidBottomNavItem(label: l10n.mainTabCourses, iconBuilder: (_, color, size, selected) => MainBottomNavKitIcons.courses(color, size, selected)),
    LiquidBottomNavItem(label: l10n.mainTabLeaderboard, iconBuilder: (_, color, size, selected) => MainBottomNavKitIcons.leaderboard(color, size, selected)),
    LiquidBottomNavItem(
