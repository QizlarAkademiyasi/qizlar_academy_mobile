...
class CoursesSearchField extends StatelessWidget {
  const CoursesSearchField({super.key, required this.controller, required this.onChanged, this.heroTag, this.readOnly = false, this.onTap, this.autofocus = false, this.focusNode, this.padding});
  ...
  /// `null` bo‘lsa — gorizontal 20. [AppBar.title] uchun odatda [EdgeInsets.zero].
  ...
  Widget build(BuildContext context) {
    ...
    final wrapped = Material(type: MaterialType.transparency, child: field);

    final heroChild = heroTag != null ? Hero(tag: heroTag!, child: wrapped) : wrapped;

    return Padding(
      padding: padding ?? const EdgeInsets.symmetric(horizontal: 20),
      // [HomeCourseCard] / [AppCourseListItemCard] bilan bir xil suyuq bosilish.
      child: AppLiquidStretch(child: heroChild),
    );
  }
}
