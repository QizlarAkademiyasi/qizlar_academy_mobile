...
// Samplers cannot be passed as function parameters by SkSL. The including
// shader binds its background image to sampler slot zero through this uniform.
uniform sampler2D uLiquidGlassBackgroundTexture;
...
vec4 calculateRefraction(vec2 screenUV, vec3 normal, float height, float thickness, float refractiveIndex, float chromaticAberration, vec2 uSize, float blurRadius, out vec2 refractionDisplacement) {
    ...
    if (chromaticAberration < 0.001) {
        ...
        return texture(uLiquidGlassBackgroundTexture, refractedUV);
    }
    ...
    float red = texture(uLiquidGlassBackgroundTexture, redUV).r;
    vec4 greenSample = texture(uLiquidGlassBackgroundTexture, greenUV);
    float blue = texture(uLiquidGlassBackgroundTexture, blueUV).b;
