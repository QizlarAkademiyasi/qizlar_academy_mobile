...
class GlassBottomBarTab {
  ...
  const GlassBottomBarTab({
    ...
    this.thickness,
          ...
          'GlassBottomBarTab requires either icon or iconBuilder',
        );
  ...
  /// Optional custom icon widget (e.g. SVG). When set, [icon] and
  /// [selectedIcon] are not used for rendering.
  ///
  /// [selected] matches the tab's visual state for this render pass
  /// (background layer uses `false`, masked layer uses `true`).
  ...
  /// [selectedIcon].
  ///
  /// Not applied when [iconBuilder] is used.
...
class _BottomBarTab extends StatelessWidget {
  ...
  List<Shadow>? _buildIconShadows() {
    ...
    if (tab.iconBuilder != null ||
        tab.thickness == null ||
        (selected && tab.selectedIcon != null)) {
      return null;
    }
